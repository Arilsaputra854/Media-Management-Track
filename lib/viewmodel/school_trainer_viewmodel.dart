...
class SchoolTrainerViewmodel extends ChangeNotifier {
  Logger log = Logger();
  ...
  void addSchool(String? schoolName) async {
    ...
    try {
      ...
      await _firestore.collection('institutions').add({'name': schoolName});
  ...
  void removeSchool(School? school) async {
    ...
    try {
      ...
      await _firestore.collection('institutions').doc(school.id).delete();
  ...
  void updateSchool(School? school) async {
    ...
    try {
      await _firestore.collection('institutions').doc(school.id).update({
        'name': school.name,
      });
      _fetchSchools();
